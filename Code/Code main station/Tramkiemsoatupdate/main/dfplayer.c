...
static void dfplayer_send_cmd(uint8_t cmd, uint16_t arg) {
    ...
    buffer[4] = 0x00; // Feedback -> DISABLED to prevent blocking due to software conflicts
    buffer[5] = (arg >> 8) & 0xFF;
    buffer[6] = arg & 0xFF;
    ...
    // Add a delay to allow the module to process the command,
    // since we are not waiting for feedback.
...
esp_err_t dfplayer_init(void) {
    ...
    // Enable internal pull-up cho TX và RX để tín hiệu ổn định hơn
    gpio_set_pull_mode(DFPLAYER_TXD, GPIO_PULLUP_ONLY);
    gpio_set_pull_mode(DFPLAYER_RXD, GPIO_PULLUP_ONLY);
