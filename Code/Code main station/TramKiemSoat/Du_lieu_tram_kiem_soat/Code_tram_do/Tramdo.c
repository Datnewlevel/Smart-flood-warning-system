...
void measurement_task(void *pvParameters) {
    ...
    ESP_LOGI(TAG, "Will measure and send data every 20 seconds.");
    ...
    while (1) {
        ...
        // 1. Heartbeat signal
        ESP_LOGI(TAG, "Heartbeat: Green LED ON for 1s");
        ...
        if (distance > 0 && distance < 450) {
            ...
            // Sensor OK signal: 2 green blinks
            ESP_LOGI(TAG, "Sensor OK signal: Green LED blinks twice.");
            gpio_set_level(LED_GREEN_PIN, 1);
            vTaskDelay(pdMS_TO_TICKS(1000));
            gpio_set_level(LED_GREEN_PIN, 0);
            ...
            if (lora_send_packet(LORA_CONTROL_STATION_ADDRESS, LORA_MEASUREMENT_STATION_ADDRESS, (uint8_t *)payload, payload_len)) {
                // LoRa Sent signal: 1 red blink
                ESP_LOGI(TAG, "LoRa sent signal: Red LED blinks once.");
            ...
            ESP_LOGW(TAG, "Measurement out of range or failed.");
            // Failure signal: Both LEDs ON for 1s
            ESP_LOGI(TAG, "Failure signal: Both LEDs ON for 1s.");
            gpio_set_level(LED_GREEN_PIN, 1);
            gpio_set_level(LED_RED_PIN, 1);
            vTaskDelay(pdMS_TO_TICKS(1000));
            gpio_set_level(LED_GREEN_PIN, 0);
...
void app_main(void)
{
    ...
    lora_write_reg(0x39, 0x34); // Using 0x34 for private networks

    // Set Preamble Length (mặc định 8)
